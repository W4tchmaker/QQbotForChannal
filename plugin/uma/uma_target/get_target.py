...
from ..uma_info import get_cn_name


# 获取育成目标的数据
async def get_tar_data(uma_name):
    cn_name = await get_cn_name(uma_name)
    print(cn_name)
    if cn_name:
        url = f'https://wiki.biligame.com/umamusume/{cn_name}'
    else:
...
async def generate_img(uma_name):
    ...
    all_height = 136
    font_path = os.path.join(f'simhei.ttf')
    ...
    for target in target_list:
        ...
        # 调整居中
        img_size = draw_img.textlength(text=target, font=img_font)
        start_width = 900 - img_size / 2
        if start_width < 0:
            start_width = 15.45
...
async def get_tar(uma_name, is_force):
    if not os.path.exists(os.path.join(resources_path, f'uma/')):
        os.mkdir(os.path.join(resources_path, f'uma/'))
    if not os.path.exists(os.path.join(resources_path, f'uma/uma_target/')):
        os.mkdir(os.path.join(resources_path, f'uma/uma_target/'))
    img_path = os.path.join(resources_path, f'uma/uma_target/target_{uma_name}.png')
